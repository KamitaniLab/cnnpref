...
if [ "$target" = '' ]; then
    echo "Please specify the network ('alexnet', 'vgg19' or 'deepsim') to be downloaded."
    exit 1
fi

# alexnet
if [ "$target" = 'alexnet' ]; then
    output=bvlc_alexnet/bvlc_alexnet.caffemodel
    srcurl=http://dl.caffe.berkeleyvision.org/bvlc_alexnet.caffemodel
...
if [ "$target" = 'deepsim' ]; then
    output=generator_for_inverting_fc6/generator.caffemodel
    ...
    cp deepsim_v0/fc6/generator.caffemodel $output
